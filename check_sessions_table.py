...
try:
    ...
    with connection.cursor() as cursor:
        # Get user_sessions structure
        cursor.execute("DESCRIBE user_sessions")
        columns = cursor.fetchall()
        
        print("user_sessions table structure:")
        print("=" * 80)
        ...
        print("\nColumn check:")
        print(f"session_uuid: {'✅' if 'session_uuid' in column_names else '❌'}")
        print(f"session_id: {'✅' if 'session_id' in column_names else '❌'}")
        print(f"start_time: {'✅' if 'start_time' in column_names else '❌'}")
